...
const Page = async () => {
  ...
  return (
    <>
      <ParticleContainer />
      <div className="container mx-auto p-5 grid grid-cols-3 p-10">
        <div className="flex flex-col justify-self-start items-center">
          ...
          <div className="flex items-center gap-1"> {location}<span className="text-white">{loc}</span></div> 
          <div className="flex gap-2 justify-center mt-4">
          {Links.map((link) => <Link href = {link.href} key={link.name} target="_blank">
            {link.icon}
            </Link>)
          }
        ...
        <div className="w-full col-span-2">
           <div className="shadow-2xl shadow-cyan-500/50 p-8 flex flex-col gap-y-4">
            <h1 className="text-4xl font-bold text-white border-b-2">{aboutMe}</h1>
            <p>{greetings}</p>
            <p>{about}</p>
          </div>
        </div>
